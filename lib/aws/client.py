...
class S3client:
    ...
    def list_obj_versions(self, obj_name: str) -> list:
        try:
            versions: dict = self.client.list_object_versions(
                Bucket=self.bucket)
    ...
    def list_objects(self, marker: str) -> list[dict]:
        try:
            stuff: dict = self.client.list_objects(
                Bucket=self.bucket, Marker=marker)
    ...
    def get_presigned_url(self, object_name: str, expiration=3600) -> str:
        try:
            response = self.client.generate_presigned_url('put_object',
